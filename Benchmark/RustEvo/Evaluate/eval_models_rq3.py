...
BASE_URL = os.getenv('BASE_URL', 'https://grid.ai.juspay.net')
...
def extract_rust_code(response: str) -> str:
    """Extract Rust code: preserve imports and the first function block."""
    ...
    for i, line in enumerate(lines):
        if re.match(r'^\s*use\s+', line):
            imports.append(line)
        if fn_start == -1 and re.match(r'\s*fn\s+\w+', line):
            fn_start = i
            break
    if fn_start == -1:
        return response
    # Grab the function until matching closing brace
    brace = 0
    body_start = None
    end_idx = fn_start
    for j in range(fn_start, len(lines)):
        ...
        brace -= lines[j].count('}')
        end_idx = j
        if brace == 0 and body_start is not None:
            break
    fn_block = lines[fn_start:end_idx+1]
    out = []
    if imports:
        out.extend(imports)
        out.append('')
...
def enforce_function_signature(code: str, signature: str) -> str:
    """Rewrite only the function signature to the required signature, preserve body and imports."""
    ...
    if fn_idx == -1:
        return code
    if brace_line_idx == -1:
        for j in range(fn_idx, len(lines)):
            if '{' in lines[j]:
                brace_line_idx = j
                break
    if brace_line_idx == -1:
        return code
    # locate end of function by balancing braces
    brace = 0
    body_start = None
    end_idx = brace_line_idx
    for k in range(brace_line_idx, len(lines)):
        brace += lines[k].count('{')
        if body_start is None and '{' in lines[k]:
            body_start = k + 1
        brace -= lines[k].count('}')
        end_idx = k
        if brace == 0 and body_start is not None:
    ...
    new_code = '\n'.join(rebuilt)
    return new_code if check_function_signature(new_code, signature) else code
...
def is_borrow_checker_error(message: str) -> bool:
    ...
    if any(p in m for p in patterns):
        return True
    # generic matcher: error[E0xxx] gets lowercased → error[e0xxx]
...
def run_rust_test(test_file_content: str, rust_version: str = "stable") -> Tuple[bool, str]:
    ...
    try:
        ...
        compile_cmd = f'rustup run {rust_version} rustc --edition=2021 --test "{temp_file_path}" -o "{output_path}"'
...
def get_code_generation_prompt(query: str, api_info: Dict[str, Any], function_signature: str) -> str:
    ...
    4. Ensure compatibility with stable Rust.
    ...
    1. Include ALL necessary import statements (use declarations) before your function.
    2. Implement ONLY the function with the given signature, no additional functions.
    3. Your implementation MUST use the specified API: {api_name} from crate {crate_name}.
    4. Ensure compatibility with stable Rust.
    5. Do not include tests, main function, or any code outside the required function and imports.
    6. Do not include additional comments or explanations.
...
def process_task(task_a: Dict[str, Any], task_b: Dict[str, Any], model: str, api_key: str, base_url: str) -> Dict[str, Any]:
    ...
    # Enforce/Check function signature
    # Try enforcement once to align header with required signature
    ...
    if api_name and not used_api:
        # Log warning but don't fail immediately - let the test determine success
        print(f"Warning: API '{api_name}' may not be used in code for task {task_a.get('task_id', 'unknown')}")
    
    # Always use stable Rust version to avoid toolchain compatibility issues
    rust_version = "stable"
