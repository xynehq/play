...
Before running automation, ensure you have:

- `pr/task_pr_<PR_NUMBER>.md` - PR task description
- `pr/original_changes.diff` - Ground truth diff

These are generated by `pr_analyzer.sh` (Stage 0).
...
### Step 1: Setup PR (One-time per PR)
...
This will:
- Fetch PR metadata
- Checkout base commit (`merge_commit^1`)
- Generate `pr/task_pr_<PR>.md`
- Generate `pr/original_changes.diff`

### Step 2: Run Automation

```bash
cd /path/to/claude-work
python3 automation_2/run_pr <PR_NUMBER> --models <MODEL_NAME>
```

**Example:**
```bash
python3 automation_2/run_pr 8916 --models glm-full
```

**Multiple models:**
```bash
python3 automation_2/run_pr 8916 --models glm-full,minimax-m2
```
...
### Stage 0: PR Setup (Manual - Run Once)

**Script**: `pr_analyzer.sh`

**Purpose**: Prepare PR data for automation

**Actions**:
1. Fetch PR metadata via `gh pr view <PR_NUMBER>`
2. Identify merge commit
3. Checkout base commit (`merge_commit^1`)
4. Create isolated branch (`test-claude-pr-<PR_NUMBER>`)
5. Generate `pr/task_pr_<PR>.md` (natural language task description)
6. Generate `pr/original_changes.diff` (ground truth diff)
...
```bash
cd /path/to/claude-work
python3 automation_2/run_pr <PR_NUMBER> --models <MODEL>
```
...
### Complete Example: PR 8916 with glm-full
